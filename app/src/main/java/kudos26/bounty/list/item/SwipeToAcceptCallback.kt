...
class SwipeToAcceptCallback(
        context: Context,
        action: (position: Int) -> Unit
) : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT) {

    private var onSwipeListener: OnSwipeListener? = null
    private var icon = context.getDrawable(R.drawable.ic_done_24dp)!!
    private val paint = Paint().apply { color = context.getColor(R.color.colorPositive) }
